...
class InferencePipeline:
    ...
    def set_pipe_and_generator(self, SDXL): 
        ...
        if SDXL:
            ...
            scheduler = EulerDiscreteScheduler.from_pretrained(self.weight_folder, subfolder="scheduler")
            self.pipe = StableDiffusionXLPipeline.from_pretrained(self.weight_folder, scheduler= scheduler,
    ...
    def generate(self, prompt: Union[str, List[str]], n_steps: int, img_sz: int, guidance_scale: Optional[float] = None) -> List[Image.Image]:
        pipe_kwargs = {
            "prompt": prompt,
